...
import { DEFAULT_HOBBIES, MIN_FRIENDS_REQUIRED, QUEST_ACCEPT_DEADLINE_HOURS } from "@/lib/constants";
import {
  formatMonthLabel,
  isDateKey,
  localDateKey,
...
import {
  CalendarDaySummary,
  CalendarQuestCompletion,
  FeedPost,
  FriendRequest,
  FriendStatus,
  LeaderboardEntry,
  MonthlyQuestCalendar,
  Notification,
  ProfileSummary,
  QuestDefinition,
  UserProfile,
...
export async function getMonthlyQuestCalendar(
  userId: string,
  options: { month?: string | null; day?: string | null } = {},
): Promise<MonthlyQuestCalendar> {
  noStore();
  await expireStaleAcceptedQuests(userId);
  ...
  const month = normalizeMonthKey(options.month, timeZone);
  const today = localDateKey(timeZone);
  const range = localMonthRange(timeZone, month);
  const selectedDay =
    isDateKey(options.day) && options.day.startsWith(`${month}-`)
      ? options.day
      : today.startsWith(`${month}-`)
        ? today
        : range.monthStartDateKey;

  const { data, error } = await supabase
    .from("user_quests")
    .select("id,quest_id,completed_at,quests(id,title,category,difficulty,xp_reward)")
    .eq("user_id", userId)
    .eq("status", "completed")
    .gte("completed_at", range.startsAt)
    .lt("completed_at", range.endsAt)
    .order("completed_at", { ascending: true });

  if (error) {
    console.error("[StreakCalendar] Failed to fetch completed quests:", error);
  }

  const completionsByDate = new Map<string, CalendarQuestCompletion[]>();
  for (const row of (data as Array<Record<string, unknown>> | null) ?? []) {
    ...
    if (!completedAt || !quest) {
      continue;
    }

    const dateKey = localDateKey(timeZone, new Date(completedAt));
    const completion: CalendarQuestCompletion = {
      id: `${row.id as string}-${completedAt}`,
      userQuestId: row.id as string,
    ...
    completionsByDate.set(dateKey, [...(completionsByDate.get(dateKey) ?? []), completion]);
  }

  const days: CalendarDaySummary[] = monthCalendarDateKeys(month).map((dateKey) => {
    const quests = completionsByDate.get(dateKey) ?? [];
    return {
      date: dateKey,
  ...
  return {
    ...
    totalCompleted: [...completionsByDate.values()].reduce((total, quests) => total + quests.length, 0),
    days,
  };
}
